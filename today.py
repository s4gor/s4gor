...
def daily_readme(birthday):
    """
    Returns the length of time since the given date (account uptime)
...
if __name__ == '__main__':
    """
    s4gor profile README generator (adapted from Andrew6rant)
    """
    ...
    age_data, age_time = perf_counter(
        daily_readme,
        datetime.datetime.fromisoformat(acc_date.replace('Z', '+00:00')).replace(tzinfo=None),
    )
    ...
    total = user_time + age_time + loc_time + commit_time + star_time + repo_time + contrib_time + follower_time
